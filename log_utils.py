...
def _make_json_serializable(obj):
    ...
    if isinstance(obj, ml_collections.ConfigDict):
        ...
        return {k: _make_json_serializable(v) for k, v in obj.items() 
                if not k.startswith('_')}  # Skip internal keys like _ErrorConfig
...
def get_flag_dict():
    ...
    for k in flags.FLAGS:
        if '.' not in k:
            try:
                value = getattr(flags.FLAGS, k)
                flag_dict[k] = _make_json_serializable(value)
            except Exception:
                # Skip flags that can't be accessed
                pass
    return flag_dict
